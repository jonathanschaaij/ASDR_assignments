...
void ConvertToRad(double &x_out_rad, double &y_out_rad, const double x_in_pixels, const double y_in_pixels)
{
  ...
  const double x_rad_per_pixel = (2 * x_limit_rad) / (2 * moving_camera_output_width);
  const double y_rad_per_pixel = (2 * y_limit_rad) / (2 * moving_camera_output_height);
...
private:
  ...
  void control_loop_timer_callback()
  {
    ...
    // Limit the setpoint
    if (setpoint_x < -x_limit_rad)
      setpoint_x = -x_limit_rad;
    if (setpoint_x > x_limit_rad)
      setpoint_x = x_limit_rad;
    if (setpoint_y < -y_limit_rad)
      setpoint_y = -y_limit_rad;
    if (setpoint_y > y_limit_rad)
      setpoint_y = y_limit_rad;
...
public:
        ...
        setpoint_x(0), setpoint_y(0)
  {
    ...
    cog_topic_ = this->create_subscription<asdfr_interfaces::msg::Point2>(
        "cog_pixels", 1, std::bind(&ClosedLoopController::cog_input, this, _1));
