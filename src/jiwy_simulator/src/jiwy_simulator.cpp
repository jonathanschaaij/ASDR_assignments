...
public:
  ...
  void set_des(const double x, const double y)
  {
    x_des_ = x;
    y_des_ = y;
  }
...
private:
  ...
  void create_topics()
  {
    ...
    webcam_input_topic_ = this->create_subscription<sensor_msgs::msg::Image>(
        "image", 10, std::bind(&JiwySimulator::webcam_topic_callback, this, std::placeholders::_1));
    moving_camera_output_topic_ = this->create_publisher<sensor_msgs::msg::Image>("moving_camera_output", 10);
    setpoint_topic_ = this->create_subscription<asdfr_interfaces::msg::Point2>(
        "setpoint", 10, std::bind(&JiwySimulator::setpoint_topic_callback, this, std::placeholders::_1));
    position_topic_ = this->create_publisher<asdfr_interfaces::msg::Point2>("position", 10);
  }
  ...
  void webcam_topic_callback(const sensor_msgs::msg::Image::SharedPtr msg)
  {
    ...
    if (center_pixel_x >= (int)msg->width)
      center_pixel_x = msg->width;
    ...
    if (center_pixel_y >= (int)msg->height)
      center_pixel_y = msg->height;
  ...
  {
    RCLCPP_INFO(this->get_logger(), "Received setpoint: [%0.2f, %0.2f]", msg->x, msg->y);
